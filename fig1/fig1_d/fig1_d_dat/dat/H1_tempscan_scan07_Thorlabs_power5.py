...
file_dict, mynames = load_fileToDictionary(genpath,r"*\scan07_S*.txt") #S* is Spectra
file_dict2, mynames2 = load_fileToDictionary(genpath,r"*\scan07_W*.txt") #W* is Wavelength
file_dict3, mynames3 = load_fileToDictionary(genpath,r"*\scan07_P*.txt") #W* is Wavelength


#%% read Thorlabs power values
##Time-Power correction from input laser av. power

power_ref = pd.read_csv(r"D:\Dropbox\TiSe2_CDW\analysis\230505_H1\Thorlabs_power5.csv",header=12)
...
tt1 = pd.read_csv(r"D:\Dropbox\TiSe2_CDW\analysis\230505_H1\scans\scan07_TEMPERATURES.txt",header=None,sep=" |\t")
tt_time = tt1.iloc[:,3]
tt_temp = []
tt_timeabs = []
nn = 0
...
# np.savetxt('H1refl_scan07_pow5_tempscan.txt',np.transpose([tt_temp,fp_in[peaks],fp_std[peaks]]))
