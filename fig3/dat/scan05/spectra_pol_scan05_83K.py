...
genpath = Path(r"D:\Dropbox\TiSe2_CDW\analysis\230504_PolScan\scan05")
...
file_dict, mynames = load_fileToDictionary(genpath,r"dat\scan05_S*.txt") #S* is Spectra
file_dict2, mynames2 = load_fileToDictionary(genpath,r"dat\scan05_W*.txt") #W* is Wavelength
file_dict3, mynames3 = load_fileToDictionary(genpath,r"dat\scan05_P*.txt") #W* is Wavelength
...
for i in temp:
    ...
    im=ax4.plot(wvl,my_norm(smooth(I_wvl_temp[aa,:],20)),c=colors[aa],lw=2,label=(str(i)+' K'))
    aa = aa+1
...
def av_DataPerTemp(x_array,mystep=5):
    x_av = []
    x_std = []
...
for pp in myindices:
    ...
    ax2.scatter(ttemp,Apeak[:,4],marker='^',lw=2,alpha=1,label=glabel[4],color=mcolors[4])
    # ax2.errorbar(tttemp,0.7*Apeak_normav[:,2],xerr=tttemp_std,yerr=0.7*Apeak_std[:,2],fmt='^',label=glabel[1],color=mcolors[1])

    ax2.scatter(ttemp,Apeak[:,6],marker='x',lw=2,alpha=1,label=glabel[6],color=mcolors[6])
...
#%% FFT filter (not great)
# Apeak_f, ff = myfft(Apeak[:,2],ttemp)

# plt.rcParams.update({'font.size': 18})
# fig21, ax21 = plt.subplots(figsize=(15,6))
# ax21.plot(ff,abs(Apeak_f)**2)
# Apeak_f[np.where(ff<0.006)] = 0
# Apeak_f[np.where(ff>0.03)] = 0

# ax21.plot(ff,abs(Apeak_f)**2)

# Apeak_a, angl = myifft(Apeak_f,ff)
# fig22, ax22 = plt.subplots(figsize=(15,6))
# ax22.plot(angl,Apeak_a,"o")
...
# np.savetxt('scan05_83K_HHlineouts.txt',(np.transpose(filetosave)),header='HWP,H3,H3Std,H5,H5Std,H7,H7Std') # not a temp scan!
